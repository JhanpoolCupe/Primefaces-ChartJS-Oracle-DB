...
public class ProductoImpl extends Conexion implements ICRUD<Producto> {
    ...
    public void guardar(Producto producto) throws Exception {
        try {
            String sql = "insert into producto" + " (NOMPRO,PREPRO,DESPRO,ESTPRO,STOPRO,IDPROV,IMGPRO)"
                    + " values (?,?,?,?,?,?,?) ";
            ...
            ps.setInt(6, producto.getIDPROV());
            ps.setString(7, producto.getIMGPRO());
            ps.executeUpdate();
            ps.close();
        } catch (Exception e) {
    ...
    public void modificar(Producto producto) throws Exception {
        try {
            String sql = "update producto set NOMPRO=?,PREPRO=?,DESPRO=?,ESTPRO=?,STOPRO=?,IDPROV=?,IMGPRO=? where IDPRO=?";
            PreparedStatement ps = dao.Conexion.conectar().prepareStatement(sql);
            ps.setString(1, producto.getNOMPRO());
            ps.setDouble(2, producto.getPREPRO());
            ps.setString(3, producto.getDESPRO());
    ...
    public List<Producto> listarTodos(int tipo) throws Exception {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                pro.setNOMPROV(rs.getString("RAZPROV"));
    ...
    public List<Producto> Graficos() throws Exception {
        List<Producto> lista = null ;
        String sql = "select * from vGRAFICOSPRO WHERE ROWNUM < 10";
        try {
            lista = new ArrayList();
            Statement st = dao.Conexion.conectar().createStatement();
            ResultSet rs = st.executeQuery(sql);
